# PRE-FREEZE TASK — Contract E RC2 Fresh Independent Reproduction v2
...
1. `research/contract_e_v1_fresh_rc2_v2_aperture/SPEC.md`
2. `research/contract_e_v1_fresh_rc2_v2_aperture/schema.json`
3. `research/contract_e_v1_fresh_rc2_v2_aperture/PRE_FREEZE_TASK.md`

Retrieve them only by exact path/ref. You may perform a branch/ref lookup to verify the execution branch points to the launch-supplied aperture head. Do not inspect the aperture commit object, tree, diff, PR, Actions, issues, history, repository search, README, or unrelated files.

If a supposedly file-scoped retrieval exposes unrelated repository/project content, stop and record `CONTAMINATED_PRE_FREEZE_APERTURE` and terminal state `INCONCLUSIVE`. Do not use the leaked material.
...
- hidden cases;
- evaluator code, qualification logic, qualification evidence, seal receipts, or comparison results;
...
General programming-language and standard-library knowledge is allowed. The implementation must be self-contained and standard-library-only.
...
`research/contract_e_v1_fresh_rc2_v2/authority_e.py`
...
`research/contract_e_v1_fresh_rc2_v2/test_authority_e.py`

Exercise the semantics you believe the public contract determines, including positive authorization, exact binding, malformed input, claimed/computed identities, currentness/revocation including fractional precision, delegation, blockers, supporting artifacts, preservation, and receipt identity.
...
4. record interpretation uncertainties;
5. record contamination status and any apparatus deviations;
6. in a subsequent metadata-only commit, create `research/contract_e_v1_fresh_rc2_v2/FREEZE_RECEIPT.json`;
7. verify that receipt commit changed no frozen implementation/test bytes;
8. stop before reveal.
...
- exact aperture file Git blobs and SHA-256 values supplied by the launch prompt;
- implementation freeze commit/path/blob/SHA-256;
- test path/blob/SHA-256;
- prereveal command/result;
- uncertainties;
