...
## 2. Hash representation boundary

Two distinct text-digest wire representations are intentionally present and must not be conflated:

- Contract-A / outer recomposition `text_sha256`: **tagged**, exactly `sha256:<64 lowercase hex>`;
- native frozen CAL V1 `result.json -> proposition.text_sha256`: **untagged**, exactly `<64 lowercase hex>`.

When comparing the native CAL field to the Contract-A / outer child binding, the exact rule is:

`"sha256:" + native_result.proposition.text_sha256 == declared_child.text_sha256`

Equivalently, the native untagged digest equals the declared tagged digest with exactly one leading `sha256:` removed.

Do not rewrite any stored wire field into the other representation. Normalize only for the explicit comparison above and for identity material defined below.
...
## 4. Exact producer/freeze authority
...
The pair must equal the exact root in `contract_a_decomposition`. The root `text_sha256` is tagged.
...
- declaration child `text_sha256` values are tagged `sha256:<64 lowercase hex>`.

## 6. Ordered child binding
...
3. outer child `text_sha256` is tagged `sha256:<64 lowercase hex>`;
4. `contract_c_content_sha256` equals the matching nested RC2 proposition `content_sha256`;
5. `contract_c_content_sha256` also equals `sha256:` + the exact native CAL result field `proposition.proposition_sha256`, where that native field is untagged 64 lowercase hex;
6. `native_result_sha256` equals `sha256:` + SHA-256 of the exact raw native child result bytes supplied independently;
7. the native child result's `proposition.proposition_id` equals the declared child;
8. the native child result's `proposition.text_sha256` is untagged 64 lowercase hex and satisfies `"sha256:" + native_text_sha256 == declared_child.text_sha256`;
9. the native child result's `result.conclusion` is exactly one of `supported | contradicted | not_checkable` and equals the outer child `conclusion`;
10. the matching nested RC2 terminal verdict equals that same child conclusion.

Do not infer a different conclusion from evidence. The native CAL result is the producer artifact under verification.

## 7. Child result identity
...
The `text_sha256` in child identity material is the **tagged Contract-A / outer child value**, not the native CAL wire representation.
...
## 9. Decomposition receipt identity
...
The receipt uses the tagged Contract-A / outer `text_sha256` values.
...
## 10. Outer local identity and canonical bytes
...
## 11. Independent whole-object authority
...
## 12. Required fail-closed families
