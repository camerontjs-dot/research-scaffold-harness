"""Frozen semantic fixtures for Contract E authority/use linearization RC2.

All identities are derived at runtime through the exact frozen Contract E
canonical-byte implementation supplied by the evaluator. The literal semantic
inputs in this file are frozen before candidate implementation.
"""
...
def intent_fixture(canonical_bytes, decision: dict[str, Any] | None = None) -> dict[str, Any]:
...
def _authority_state(canonical_bytes, intent: dict[str, Any], *, revoked_at: str | None) -> dict[str, Any]:
    ...
    target_ref = sha256_bytes(canonical_bytes(reference_identity_payload))
    state = {
...
def authority_a1(canonical_bytes, intent: dict[str, Any] | None = None) -> dict[str, Any]:
    intent = intent_fixture(canonical_bytes) if intent is None else copy.deepcopy(intent)
    return _authority_state(canonical_bytes, intent, revoked_at="2026-09-04T12:00:00Z")
...
def authorization_request(canonical_bytes, intent: dict[str, Any], authority_state: dict[str, Any]) -> dict[str, Any]:
    ...
    ref_identity = sha256_bytes(canonical_bytes(ref_payload))
    return {
